...
int queryHash(int l,int r,int k) {
    ...
    for(int i=0;i<=n;i+=k) {
        ...
        if(lx>rx) continue;
        if(id[lx]==id[rx]) res=Inc(res,Mul(Dec(preSB[id[lx]][rx],preSB[id[lx]][lx-1]),Mul(inv[lx-1],fac[lx-i])));
        else {
            int cnt=0;
            ...
            res=Inc(res,Mul(cnt,Mul(inv[lx-1],fac[lx-i])));
        }
    }
    return res;
}
...
signed main() {
...
#endif
    read(n,q);
    for(int i=1;i<=n;i++) read(a[i]),a[i]=(a[i]%mod+2*mod)%mod;
    ...
    while(q--) {
        ...
        if(ops==1) {
			...
			cout << (cnt1%mod+2*mod)%mod << " " << (cnt2%mod+2*mod)%mod << endl;
