...
bool check(int mid) {
	...
	for (int i = n; i >= 1; i--) {
		...
		pair <int, bool> cur = query(pos);//当前点所在集合
		points.push_back(cur.first);
		inter[cur.first] = make_pair(1, k);
		for (auto x : a[pos]) {
			pair <int, bool> tmp = query(x);
			if (inter[tmp.first].first == 0) {
				points.push_back(tmp.first);//son所在集合
				inter[tmp.first] = make_pair(1, k);//集合的区间限制
			}
			if (tmp.second == cur.second) chkmax(inter[tmp.first].first, dp[x] + 1);//同类边，[dp[x]+1, k]
			else chkmin(inter[tmp.first].second, k - dp[x]);//异类边，[1, k-dp[x]]
		}
		pair <int, int> ban = make_pair(k + 1, 0);//禁止区间
		int l = inter[cur.first].first, r = inter[cur.first].second;//相同集合边的区间限制 = [l, r]
		for (auto x : points) {//枚举所有相邻集合的2个区间限制
