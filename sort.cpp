		...
		rep(i, 1, MAXN){
			...
			if(num[i] >= __){
				if(i == sel[0]){
					...
					pair <int, int> tmp = make_pair(' ', COLOR_DARKRED * COLOR_BG);
					if (tmp != rd[__][i][fcnt]) update(__, i, tmp);
					rd[__][i][fcnt] = tmp;
				}
				else if (i == sel[5]) {
					pair <int, int> tmp = make_pair(' ', COLOR_PURPLE * COLOR_BG);
					if (tmp != rd[__][i][fcnt]) update(__, i, tmp);
					rd[__][i][fcnt] = tmp;
				}
				else if (i == sel[6]) {
					pair <int, int> tmp = make_pair(' ', COLOR_DARKYELLOW * COLOR_BG);
					if (tmp != rd[__][i][fcnt]) update(__, i, tmp);
					rd[__][i][fcnt] = tmp;
				}
				else if (i == sel[7]) {
					pair <int, int> tmp = make_pair(' ', COLOR_DARKGREEN * COLOR_BG);
					if (tmp != rd[__][i][fcnt]) update(__, i, tmp);
					rd[__][i][fcnt] = tmp;
				}
				else if (i == sel[8]) {
					pair <int, int> tmp = make_pair(' ', COLOR_DARKBLUEGREEN * COLOR_BG);
					if (tmp != rd[__][i][fcnt]) update(__, i, tmp);
					rd[__][i][fcnt] = tmp;
				}
				else if (i == sel[9]) {
...
restart:
	rep (i, 1, MAXM) rep (j, 1, MAXN) rd[i][j][0] = rd[i][j][1] = make_pair(' ', COLOR_WHITE);
	system("cls");
	cls();
	do {
	...
	do {
		...
		cout << "8. 堆排序 (O(nlogn ~ nlogn) 不稳定)" << endl;
		cin >> mode;
		system("cls");
	} while(mode < 1 || mode > 8);
	init();
