...
struct node {
	...
	node() {}
	node(int _x, int _y) : first(_x), second(_y) {}
};

node make_pair(int x, int y) {
	return node(x, y);
}
...
int main() {
	...
	//mad 不写了。
	multiset <node> impact;
	for (auto x : genshin) {
		impact.insert(make_pair(x.second, x.first));
	}
	rep (i, 1, q) {
		multiset <node> CTRnmsl;
		int x, y;
		read(x), read(y);
		if (x > y) swap(x, y);
		if (x == 1) {
			...
			print(CTRnmsl.begin() -> second), putchar(10);
		}
		else {
			if (c[x] == c[y]) print(c[x]), putchar(10);
			else print(min(c[x], c[y])), putchar(10);
		}
	}
	return 0;
}