...
public:
	...
	node* merge(node* cura, node* curb) {
		return merge(cura, curb, 1, 1e5);
	}

	node* modify(node* cur, int pos, int val) {
		return modify(cur, 1, 1e5, pos, val);
	}
};
...
signed main() {
	...
	rep (i, 1, m) {
		...
		root[a] = add(root[a], c, 1);
		root[b] = add(root[b], c, 1;
		root[lca = TCS::LCA(a, b)] = add(root[lca], c, -1);
		root[TCS::fa[lca]] = add(root[TCS::fa[lca]], c, -1);
	}
	return 0;
}
