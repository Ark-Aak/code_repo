...
bool parse(string cmd,int execLine){
	if(regex_match(cmd,m,regexs::print)){
		...
		smatch varA, varB, varC;
		string varAName = m.str(1), varBName = m.str(2);
		if (!regex_match(varAName, varA, regexs::var) || !regex_match(varBName, varB, regexs::arr)) goto error;
		string varCName = m.str(3);
		if (!regex_match(varCName, varC, regexs::var)) goto error;
		if (!v.hasVar(varA.str(1)) || !arrs.hasArr(varB.str(1)) || !v.hasVar(varC.str(1))) goto error;
		int val = v.getInt(varA.str(1), mark);
		if (mark) goto error;
		if (v.getType(varA.str(1)) != INT) goto error;
		if (v.getType(varC.str(1)) != INT) goto error;
		int pos = v.getInt(varC.str(1), mark);
		if (mark) goto error;
		cout << "put arr pos = " << pos << endl;	
		if (mark) goto error;
		...
		int pos = v.getInt(varC.str(1), mark);
		if (mark) goto error;
		cout << "get arr pos = " << pos << endl;	
		if (mark) goto error;
		int val = arrs.get(varB.str(1), pos, mark);
		if (mark == 1) goto error;
		if (mark == 2) goto runtime_error;
		v.setVarInt(varA.str(1), val, mark);
		if (mark) goto error;
	}
