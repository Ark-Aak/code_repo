...
int main() {
	...
	while (T --> 0) {
		...
		if (n % 2 == 0 && m % 2 == 0) {
			...
			if ((int) ans.size() < n) ans.emplace_back((n - sum) / 2), ans.emplace_back((n - sum) / 2);
			if (sum != n) {
				puts("-1");
				continue;
			}
			...
			if ((int) ans.size() < n) rep (i, 1, n - 2) ans.emplace_back(1), sum++;
			if ((int) ans.size() < n) ans.emplace_back((m - sum) / 2), ans.emplace_back((m - sum) / 2);
			if (sum != n) {
				puts("-1");
				continue;
			}
