...
int main() {
	...
	while (T --> 0) {
		...
		if (n % 2 == 0 && m % 2 == 0) {
			...
			m -= 5, n -= 2;
			rep (i, 1, n - 2) ans.emplace_back(2), sum += 2;
			if (n - sum < 4) {
				puts("-1");
				continue;
			}
			ans.emplace_back((n - sum) / 2), ans.emplace_back((n - sum) / 2);
			...
			if (ans.size() < n) rep (i, 1, n - 2) ans.emplace_back(1), sum++;
			if (ans.size() < n) ans.emplace_back((m - sum) / 2), ans.emplace_back((m - sum) / 2);
			for (auto i : ans) print(i), putchar(32);
			putchar(10);
		}
		else {
			...
			rep (i, 1, n - 2) ans.emplace_back(1), sum++;
			ans.emplace_back((m - sum) / 2), ans.emplace_back((m - sum) / 2);
			for (auto i : ans) print(i), putchar(32);
			putchar(10);
		}
		/*
