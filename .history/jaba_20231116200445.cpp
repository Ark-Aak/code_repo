...
bool parse(string cmd,int execLine){
	if(regex_match(cmd,m,regexs::print)){
		...
		if (!regex_match(labelname, regexs::labelname)) goto error;
		int gotoLine=labelLine[labelname];
		cout << labelname << " " << gotoLine << endl;
	...
	else if(regex_match(cmd,m,regexs::ing)||regex_match(cmd,m,regexs::endif)){
		return 0;
	}
	else if (regex_match(cmd, m, regexs::label)) {
		labelLine[m.str(1)] = execLine;
	}
	else{
